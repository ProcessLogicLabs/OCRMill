...
class CoexpoIcatSaTemplate(BaseTemplate):
    ...
    def extract_line_items(self, text: str) -> List[Dict]:
        ...
        1. Single-line format (pdfplumber):
           'SK01YLC 71132842 5.83 Knit Unisex Polo Shirt 100PL 90.738 529.31 24.00 ICAT S.A DE C.V'
        2. Multi-line format (PyMuPDF/fitz): Each field on a separate line
        ...
        # Try primary single-line pattern
        items = self._parse_with_pattern(text, pattern_type='primary')

        # If no items found with single-line, try multi-line parsing
        if not items:
            items = self._parse_multiline(text)
        ...
        if expected_totals:
            ...
            if mismatched:
                ...
                if not alt_items:
                    alt_items = self._parse_multiline(text)
    ...
    def _extract_invoice_totals(self, text: str) -> Dict[str, float]:
        ...
        for i, line in enumerate(lines):
            ...
            # Look for TOTAL line: "184.08 TOTAL 13,917.84 637.82" (single-line format)
            ...
            # Also handle multi-line format where TOTAL is on its own line
            # Pattern: line with just a number, then "TOTAL", then another number
            if line == 'TOTAL' and i > 0 and i < len(lines) - 1:
                # Look back for dozen value and forward for total value
                prev_line = lines[i-1].strip()
                next_line = lines[i+1].strip() if i+1 < len(lines) else ""
                try:
                    # Previous line should be dozen total
                    dozen_val = float(prev_line.replace(',', ''))
                    # Next line should be amount total
                    amount_val = float(next_line.replace(',', ''))
                    if current_invoice != "UNKNOWN" and amount_val > 0:
                        totals[current_invoice] = amount_val
                except ValueError:
                    pass

        return totals

    def _parse_multiline(self, text: str) -> List[Dict]:
        """Parse line items from multi-line format (PyMuPDF/fitz extraction).

        Multi-line format has each field on a separate line:
        Line 0: Style code (e.g., 11FP67230PES01VS1100WHTU)
        Line 1: PO number (e.g., PO0049884-5)
        Line 2: Dozen (e.g., 0.08)
        Line 3: Description (e.g., Woven 0 Fanny pack 99PL/1CF)
        Line 4: Cost (e.g., 43.059)
        Line 5: Total (e.g., 3.59)
        Line 6: Weight (e.g., 1.00)
        Line 7: Producer (e.g., VIDALES LARRAÑAGA, S.A. DE C.V)
        Line 8: Base style (e.g., 11FP-6723) - optional, may not be present
        """
        items = []
        lines = text.split('\n')
        ...
        # Style code pattern - alphanumeric, 15+ chars, starts with digit or letter
        style_pattern = re.compile(r'^(\d{2}[A-Z]{1,2}[A-Z0-9-]{10,})$|^(SK[A-Z0-9]+)$', re.IGNORECASE)
        # PO pattern
        po_pattern = re.compile(r'^(PO\d{7,}[-\d]*)$|^(\d{8})$', re.IGNORECASE)
        # Decimal number pattern (for dozen, cost, total, weight)
        decimal_pattern = re.compile(r'^[\d,]+\.?\d*$')
        # Description pattern - starts with Woven or Knit
        desc_pattern = re.compile(r'^(Woven|Knit)\s+', re.IGNORECASE)
        # Producer pattern
        producer_pattern = re.compile(r'^(VIDALES|ICAT)', re.IGNORECASE)
        ...
        while i < len(lines):
            ...
            # Skip header lines
            if line.upper() in ['STYLE', 'CUT', 'DOZEN', 'DESCRIPTION', 'COST', 'TOTAL', 'WEIGHT', 'PRODUCER NAME']:
                i += 1
                continue
            ...
            if any(skip in line.upper() for skip in ['BOXES TOTAL', 'ROLLS FABRICS', 'RETURN PACKING',
                                                      ...
                                                      'DR-CAFTA', 'ELIGIBLE', 'EXPORTER', 'SHIPPER']):
                i += 1
                continue

            # Try to match a style code (start of a new item)
            style_match = style_pattern.match(line)
            if style_match and i + 6 < len(lines):
                raw_style = style_match.group(1) or style_match.group(2)

                # Look ahead for the remaining fields
                try:
                    po_line = lines[i + 1].strip()
                    dozen_line = lines[i + 2].strip()
                    desc_line = lines[i + 3].strip()
                    cost_line = lines[i + 4].strip()
                    total_line = lines[i + 5].strip()
                    weight_line = lines[i + 6].strip()
                    producer_line = lines[i + 7].strip() if i + 7 < len(lines) else ""

                    # Validate PO
                    po_match = po_pattern.match(po_line)
                    if not po_match:
                        i += 1
                        continue
                    cut = po_match.group(1) or po_match.group(2)

                    # Validate dozen
                    if not decimal_pattern.match(dozen_line):
                        i += 1
                        continue
                    dozen = float(dozen_line.replace(',', ''))

                    # Validate description
                    if not desc_pattern.match(desc_line):
                        i += 1
                        continue
                    description = desc_line

                    # Validate cost
                    if not decimal_pattern.match(cost_line):
                        i += 1
                        continue
                    cost = float(cost_line.replace(',', ''))

                    # Validate total
                    if not decimal_pattern.match(total_line):
                        i += 1
                        continue
                    total = float(total_line.replace(',', ''))

                    # Validate weight
                    if not decimal_pattern.match(weight_line):
                        i += 1
                        continue
                    weight = float(weight_line.replace(',', ''))

                    # Producer is optional but helpful
                    producer = ""
                    if producer_pattern.match(producer_line):
                    ...
                    if quantity_unit == 'PRS':
                        quantity = round(dozen * 12)  # Convert dozens to pairs
                    else:
                        quantity = round(dozen, 2)  # Keep 2 decimal places for dozens

                    items.append({
                        'invoice_number': current_invoice,
                        'part_number': base_style,
                        'raw_part_number': raw_style,
                        'cut_number': cut,
                        'quantity': quantity,
                    ...
                    # Skip the lines we just processed (style + 7 fields + optional base style)
                    i += 8
                    # Check if next line is the base style (has hyphen and is short)
                    if i < len(lines) and '-' in lines[i].strip() and len(lines[i].strip()) <= 12:
                        i += 1
                    continue
    ...
    def _parse_with_pattern(self, text: str, pattern_type: str = 'primary') -> List[Dict]:
        ...
        if pattern_type == 'primary':
            line_item_pattern = re.compile(
                ...
                r'([\d,]+\.?\d{2,3})\s+'         # COST (like 90.738, 71.514, or 7,738.670 - can have commas)
            ...
            line_item_pattern = re.compile(
                ...
                r'([\d,]+\.?\d{2,3})\s+'         # COST (can have commas)
                r'([\d,]+\.\d{2})\s+'            # TOTAL
